...
local doc = xml.decode([[<foo><bar><a empty="" /><a><only-child/></a><b/>c</bar><baz><!-- --></baz><qux/></foo>]])
assert(doc:query("foo > bar"):count() == 4)
assert(doc:query("foo > bar"):count("a") == 2)
assert(doc:query("foo > bar"):count("b") == 1)
assert(doc:query("foo > bar"):count("c") == 0)
assert(doc:query("foo > baz"):count() == 0)
assert(doc:query("foo > qux"):count() == 0)
assert(doc:query(":empty"):attr("empty") == "")
assert(doc:query(":only-child"):name() == "only-child")
assert(doc:query("bar > :only-of-type"):name() == "b")
assert(doc:query_all("bar > *"):count() == 3)
assert(doc:query_all("bar > *"):count("a") == 2)
assert(doc:query_all("bar > *"):count("b") == 1)
assert(doc:query_all("bar > *"):count("c") == 0)
